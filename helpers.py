...
def translate_chunk(to_translate, translated_list, src_language):
    ...
    :param translated_list: the list to which to append the translated result
    :param src_language: the language of the text to translate
    """
...
def replace_ngram(text, bigrams):
    """
    Replaces words with bigrams in text
    """
    for gram in bigrams:
...
def generate_wordcloud(data, title, bigrams=True):
    ...
    :param data: the word frequencies used to build the word cloud
    :param title: plot title
    :param bigrams: whether to include bigrams in the word cloud
    """
...
def visualize_projection_single(section, df_recipes, projection_method="pca", clusters=[0] * 150):
    """
    Projects the TF-IDF of a section to 2 dimensions and then displays a plot of the result
    ...
    palette = (
        "#000000", "#009292", "#ff6db6", "#490092", "#006ddb", "#b66dff", "#6db6ff", "#b6dbff", "#920000", "#24ff24",
        "#db6d00", "#ffff6d")
...
def get_clusters(section, df_recipes, num_clusters=2, clustering_method="kmeans", eps=10, min_samples=20):
    """
    Computes clusters in a section of the recipes using the given method
    :param section: name of the section to visualize
    :param df_recipes: dataframe containing the sections to project
    :param num_clusters: the number of clusters to use when running K-means clustering
    :param clustering_method: either "kmeans" or "dbscan"
    :param eps: the eps parameter for DBSCAN
    :param min_samples: the min_samples parameter for DBSCAN
    :return: a list containing the cluster label for each datapoint
    """
    vectorizer = TfidfVectorizer()
    response = vectorizer.fit_transform(df_recipes[f"preprocessed_{section}"])
    X = response.toarray()
